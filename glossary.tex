...
{
  name=HTTPS,
  description={HTTPS, ausgeschrieben HyperText Transfer Protocol Secure ist ein
              Protokoll, welches zur verschlüsselten Übertragung von Inhalten,
              wie z.B. Webseiteninhalte verwendet wird.}
...
\newglossaryentry{LDAP}
{
  ...
  description={Lightweight Directory Access Protocol, ist wie auch \gls{HTTPS}
            ...
            LDAP ist in diesem Fall das ungesicherte Protokoll, LDAPs hingegen
            das gesicherte.}
}
\newglossaryentry{Active Directory}
{
  name=Active Directory,
  description={Ist ein von Microsoft entwickleter Server Dienst, welcher auf
...
{
  name=Treiber,
  description={Ein Treiber definiert die Schnittstelle zwieschen einer
              Software und einer Hardware. Damit eine gewisse Software auf 
              eine Hardware zugreifen kann, wird ein sogenannter Treiber
              verwendet.}
}
...
{
  name=Dashboard,
  description={Ein Dashboard ist eine Form auf einer Webseite zum Beispiel
              um Graphen oder Metriken einem User zur Verfügung zu stellen.
              Sie werden hier dem Anwender grafisch dargestellt.}
}
\newglossaryentry{Templates}
{
  name=Templates,
  description={Ist eine Vorlage, die für Webgrafiken oder ganze Webseiten
              dient. Ebenfalls können Templates für eine Abfrage von Graphen
              dienen. Hier sind dann meist bereits schon Beispiel Werte
              eingetragen.}
}
...
\newglossaryentry{Agents}
{
  ...
  description={Ist meist ein Programm, welches bestimmte Abläufe selbständig
              ohne weitere Eingriffe des Users überwachen kann. Sobald
