...
Bevor wir das Projekt beginnen konnten, mussten vorab zusammen mit dem
Projektteam, eine Softwareliste an möglichen Frontends erarbeitet werden.
Bereits vorab wurden folgende Softwarelösungen in das Pflichtenheft aufgenommen:
\begin{outline}
  \1 Grafana
  \1 Graphite
  \1 Zabbix-Frontend
...
\begin{outline}
  ...
  \1 Netdata
\end{outline}
Das Hinzufügen, von weiteren
Softwarelösungen, während der Evaluierungsphase bedarf keine weitere Zustimmung
durch den Auftragegeber, sondern konnte direkt mit dem Projektteam besprochen
und aufgenommen werden. Zusätzlich zu der Liste mit den einzelnen
Softwarelösungen, wurden verschiedene Anforderungen in einem
...
Softwareliste besteht zum Teil aus Softwarelösungen, welche im eigenen
Unternehmen, bereits erfolgreich eingesetzt worden sind und zum anderen
von Vorschläge des Auftraggebers.

Das Hinzufügen von weiteren Softwarelösungen, während der Evaluierungsphase
bedarf keine weitere Zustimmung durch den Auftraggeber.
...
Damit die Evaluierung der einzelnen Softwarelösungen durchgeführt werden konnten,
mussten zuvor Kriterien mit dem Projektteam in einen
Anforderungskatalog aufgenommen werden. Diese Kriterien, in Form von definierten
Anforderungen, sollen für eine besser Übersicht, während der Evaluierung dienen. Bei 
den Kriterien, musste darauf geachtet werden, dass diese während der Beurteilung
der Anforderungen eine Machbarkeit und Realistik besitze. Nachdem die einzelnen
...
\begin{outline}
  ...
  Weboberfläche selber vor ungeschützten Zugriff durch dritter geschützt sein
  muss. Ebenfalls muss die Verbindung, welche das Frontend nutzt, um die Daten
  abgreifen zu können vor dritter so geschützt und gesichert werden, das hier
  kein Schaden verursacht werden kann. Der Zugriff auf die Weboberfläche soll
  über ein gesichertes HTTPS Protokoll durchgeführt werden können und je nach
  Kundenanforderung auch eigene Zertifikate zulassen.
  Die Anmeldung am Frontend kann entweder über einen Lokalen Administrator, als
  auch über eine LDAP(Lightweight Directory Access Protocol)
  ...
  sollen über z.B. ein externes System (FileServer o.a.) abgelegt werden
  ...
  Diese Plugins werden, bzw sollen in Javascript geschrieben werden können.
  Die Weboberfläche sollte jedoch auch andere Sprachen unterstützen, um so eine
  Unabhängigkeit von Entwicklern gewährleisten zu können. Unternehmenen müssen
  so nicht auf spezielle Entwickler von bestimmten Sprachen zurückgreifen.
\end{outline}
\mr%
