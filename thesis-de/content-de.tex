...
\begin{tabular}{lccccc}
  \toprule
  System & Modifizierbarkeit & Dokumentation & Wissen & Abhängigkeit & Gesamt \\
  \midrule
...
gestarteten virtuellen Maschine oder Container mit einem Betriebssystem auf die
Funktionalität der einzelnen Module. Hierzu wurden schon bereits vorhandene
Test Szenarien von \texttt{testkitchen} und \texttt{serverspec} verwendet und
diese auf die im Projekt verwendeten Module angepasst. Der Code wird innerhalb
dieser Tests ausgeführt und geprüft, ob die Funktionalität der einzelnen Module
für den Benutzer gegeben ist. Diese Szenarien prüfen beispielsweise, ob die
Erhaltung der Daten, sowie speichern dieser Daten in der Datenbank und
Visualisierung der Graphen funktioniert. Es wird hier nicht geprüft, ob das
System stabil läuft sondern lediglich, ob die einzelnen Funktionen vorhanden
sind und funktionieren. Die Ausgabe erfolgt hier entweder in Bestanden oder
nicht Bestanden. Anschließend kann der Entwickler diesen Code erneut anpassen
und testen. Nachfolgend ist ein Beispiel für einen Akzeptanztest für das Modul
Collectd angehangen.

\begin{listing}[H]
  \inputminted{ruby}{../listings/acceptance-test.txt}
  \caption{Auszug aus einem Akzeptanztest für Collectd}
\end{listing}

Hier kann man sehen, das eine \gls{cURL} Abfrage auf das Collectd Modul
durchgeführt wird und geprüft wird, ob die Werte, welche zuvor definiert wurden
ausgegeben werden. Wie auch für die~\ref{sec:unit-tests} werden Akzeptanztests
für verschiedene Betriebsysteme programmiert. Anders als
bei~\ref{sec:unit-tests} werden diese nicht in einer \texttt{Metadata.json}
hinterlegt, sondern jedes Betriebssystem erhalt eine eigenen Datei, in der
definiert ist, mit welcher virtuellen Maschine oder Container dieser Code
