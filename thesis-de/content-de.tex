...
vielen Datenvisualisierungs-Systemen ohne zusätzliche Entwicklungskosten
...
Analyse der Datenvisualisierung). Alle Produkte sind unter der GPL2 lizenziert.
Der Agent ist in C geschrieben. Er bietet die Möglichkeit, diverse Daten direkt
aus dem lokalen System auszulesen und an den zabbix-server zu senden. Das
Webinterface ist eine PHP-Applikation, welche die Daten visualisiert und direkt
aus der Datenbank liest. Der Agent kann mit externen Skripten erweitert werden.
In Kombination mit Coreutils kann er alle benötigten Informationen auslesen.
Die Community bietet dafür fertige Skripte an
...
überprüft. Sobald diese bei 75°C oder höher liegt, wird eine E-Mail an
...
In der Shell-Ausgabe unter Listing~\ref{lst:cassandra} wird ein während der
Evaluierungsphase erstelltes Cassandra-System dargestellt. Dabei besitzt der
Prototyp zwei aktive Nodes, welche für Testzwecke auf dem gleichen physischen
Server betrieben werden. Im Produktivbetrieb sollten diese auf zwei
...
Bei der Evaluierung der API-Systeme wurden die
...
Dabei schaute sich Herr Luis speziell die Syntax zwischen den beiden
Softwareprodukten an. Die Syntax definiert dabei den Aufbau einer Datenanfrage
und daraus resultierende Datenzulieferung. Beide Definitionen muss die API eins
zu eins übernehmen, andernfalls wäre eine Kommunikation der beiden
Softwareprodukte nicht möglich.

Neben der technischen Syntax wurde ebenfalls die Informationsinhalte, welche
die API von der Datenvisualisierung erhält und zurück sendet, auf den
...
einen Administrator, welcher zum Beispiel alle Server sehen darf, oder ob Daten
an einen Kunden, welcher zum Beispiel nur seinen eigenen Server sehen darf,
zurück gesendet werden sollen.

Um diese Problematik zu lösen, hat das Projektteam den sogenannten
„Header“-Bereich (auf Deutsch Kopf-Bereich), welcher bei jeder Internetanfrage
vorhanden ist, genauer untersucht. In diesem steht in der Regel die Herkunft
der Anfrage und weitere Informationen wie der genutze Browser. Speziell bei
der Anfrage von der Datenvisualisierung an die API steht in dem Header-Bereich
immer die Organisations-ID von dem anfragenden Benutzer. Die Organisations-ID
ist bei der für das Projekt ausgewählten Datenvisualisierungs-Software eine
Abgrenzung der Zugriffsrechte. Das heißt sie definiert, welche Daten der
eingeloggte Benutzer sehen darf und welche nicht.
...
Organisations-ID zu welchen Server gehört. Sollte die Datenvisualisierung
Daten bei der API anfordern, würde diese vor der eigentlichen Datenabfrage
bei der Datenhaltung die vorhandene Organisations-ID in den Server-Namen
übersetzen. Mit dieser Übersetzung kann die API anschließend regulär die
Daten von der Datenhaltung anfordern und an die Datenvisualisierung liefern.
...
diese ID in irgendeiner Weise zu verändern und somit andere Daten zu erhalten.
\nl%
...
\begin{outline}
  ...
  LDAP-Anbindung (Lightweight Directory Access Protocol) verfügen, um hier z.B.
  ...
  \1 Die verwendete Datenvisualisierungssoftware muss mit einem API-System
  interagieren können. Dies dient der Sicherheit, da innerhalb der
  Datenvisualisierung die Datenhaltungsanfragen nicht eigenmächtig modifiziert
  werden können. Entsprechend können Daten in der Datenhaltung nicht verändert,
  angepasst oder sogar gelöscht werden. Zum anderen kann über das API-System
  durch verschiedene Methoden die Performance der Datenvisualisierung
  verbessert werden.
  \1 Plugin-Erweiterung. Die Datenvisualisierung muss eine Plugin-Erweiterung
...
Neben den angesprochenen Inhalten wurde im Transformations-Teil eine
Aggregation der Daten zugunsten des Speicherplatzes definiert. Dabei
vergleicht der Algorithmus den neuen Datensatz mit dem aktuellsten
Datensatz in der gleichen Stunde. Voraussetzung dabei ist, dass der
aktuellste Datensatz in der Datenhaltung die gleichen Eigenschaften besitzt wie
der neue Datensatz. Sollte dabei der Wert „Value“, dass ist der Wert
in welchem der aktuelle Verbrauch der CPU, RAM, et cetera, identisch sein,
wird der neue Datensatz fallen gelassen und nicht in die Datenbank geschrieben.
Dies hat den Effekt, dass wenn der Server zum Beispiel für 1 Minute eine Auslastung
von Null Prozent besitzt, für diese Minute nur ein einziger Datensatz in der
Datenhaltung existiert. Die dort fehlende Werte kann man entsprechend bei der
Ausgabe zurück rechnen. Dadurch das jede Stunde mindestens ein Datensatz von
jedem angebunden Server in die Datenhaltung geschrieben wird, kann schnell
zwischen einem ausgeschaltenden Server und einem Server ohne Nutzung
...
Inhalt. Neben diesen vier URL's wurde ebenfalls das extrahieren der in
Abschnitt~\ref{subsec:zusammenspiel_api_frontend} erläuterten
Organisations-ID implementiert.
...
Instanz vorgesehen. Durch die in Punkt ~\ref{subsec:zusammenspiel_api_frontend}
erläuterte Mapping-Tabelle wird sichergestellt, dass der Benutzer nur die
zu Ihm zugeordneten Daten sehen kann. Administratoren oder andere Mitarbeiter
eines Unternehmens, wie zum Beispiel ein Kundenberater, erhalten mehrere
Zuordnungen von Organisations-IDs. Somit erhalten diese Zugriff auf die
Dateninhalte des Kunden und können somit Ihre Aufgaben, wie die Beratung
auf ein neues Produkt, oder Verbesserung der Gesamtperformance, durchführen.
Andersherum muss für einen Kunden mit mehreren gemieteten virtuellen Instanzen
kein neuer Benutzer angelegt werden, stattdessen wird seinem bestehenden
Benutzer eine weitere Organisations-ID hinzugefügt.
...
Die Tests für das Grafana-Profil befinden sich unter
...
besitzt betriebssystemspezifische \glslink{Fact}{Facts}. Diese werden von rspec
