...
Messpunken betrachtet.  Beispiel: Das Betriebssystem misst die geschriebene
...
als Open Source Software. Der Linux Kernel stellt Informationen über
...
ermitteln~\cite{procps}. Über das Programm free oder die Datei /proc/meminfo
erhält man den die Menge des vorhandenen sowie zur Zeit benutzten
Arbeitsspeichers. Die Anzahl der Prozessorkerne liefert das Programm nproc oder
die Datei /proc/cpuinfo. /proc/diskstats liefert Informationen über die Anzahl
der Schreib- und Lesevorgänge auf allen Datenträgern. Außerdem listet die Datei
die geschriebe als auch gelesene Datenmenge. Die Informationen beziehen sich
auf die Zeit seit dem letzten Startvorgang des Servers bis zum ausgeben der
Datei. Ein kontinuierliches Ausgeben ermöglicht die Berechnung des Durchsatzes
pro Sekunde. Diese Aufgabe wird von der Datenbank übernommen und ist näher im
Abschnitt~\ref{subsubsection:Begriffserklärung} erklärt. Das gleiche gilt für
die Datei /proc/net/dev. Sie listet alle Netzwerkadapter des Systems
...
Daten des Netzwerkadapters ``enp4s0'' in Bytes auszulesen:

\begin{lstlisting}[language=bash,caption={Parsing /proc with awk}]
#!/bin/bash
echo "device,receive,transmit"
while :; do
  awk '/enp4s0/ {print "enp4s0,"$2","$10}' /proc/net/dev
  sleep 60
done
\end{lstlisting}
...
\begin{lstlisting}[language=bash,caption={traffic stats enp4s0}]
device,receive,transmit
enp4s0,28932776123,1786084537
enp4s0,28935220128,1786163375
enp4s0,28939447268,1786302393
enp4s0,28942260029,1786392663
enp4s0,28946985781,1786547807
enp4s0,28949168552,1786622616
enp4s0,28951976308,1786708990
\end{lstlisting}
...
\begin{tabular}{lcr}
  \toprule
  Typen                        & Coreutils & Collectd \\
  \midrule
  Zugewiesener Arbeitsspeicher & \cmark    & \xmark \\
  Anzahl Prozessorkerne        & \xmark    & \cmark \\
  Datenträger Durchsatz        & \xmark    & \cmark \\
  Netzwerk Durchsatz           & \xmark    & \cmark \\
  \bottomrule
\end{tabular}
\end{center}
\tm%
