...
Bevor das Projekt beginnen konnten, musste zusammen mit dem Auftraggeber und
dem Projektteam, eine Liste an möglichen Frontends erarbeitet werden.
Zusätzlich zu dieser Liste, wurden verschiedene Anforderungen definiert, welche
im späteren Verlauf der Planung für die Evaluierung der einzelnen Frontends
verwendet wird. Diese Liste besteht zum Teil aus Software Lösungen, welche im
eigenen Unternehmen, bereits erfolgreich eingesetzt worden sind und zum anderen
von Vorschläge des Auftraggebers. Während der Planung des Frontends, kamen noch
weitere Frontends zu der Liste hinzu, welche in die Evaluierung mit aufgenommen
worden sind.

Dies benötigte keine Zustimmung des Auftraggebers, sondern konnte mit dem
Projektteam besprochen werden.

Folgende Frontends wurden während der Planung, bereits vorab mit dem
Auftraggeber und dem Porjektteam erarbeitet:
...
Damit die Evaluierung der einzelnen Frontends durchgeführt werden konnten,
mussten zuvor Kriterien zusammen mit dem Porjektteam definiert werden, welche
die Frontends erfüllen müssen. (Diese sollen später für eine bessere
Übersichtlichkeit dienen, welche Anforderungen das Frontend haben muss). Bei 
den Kriterien, musste darauf geachtet werden, dass diese zu Projektübergabe
realistisch und machbar durchgeführt werden konnten. Nachdem die einzelnen
Kriterien zusammen mit dem Projektteam erstellt worden sind, wurden diese im
Anschluss von jedem einzelnen Projektmitglied auf die Machbarkeit und
Durchführung geprüft. Erst hiernach wurden diese der Kriterien Liste
aufgenommen.

Die einzelnen Kriterien wurden im nachhinein der Wichtigkeit absteigend,
wie folgt aufgelistet:
\begin{outline}
  \1 Es muss eine Authentisierung, Autentifizierung und Autorisierung
  gewährleistet werden. Dies bedeutet, dass die Daten und damit auch die View
  vor ungeschützten Zugriff dritter geschützt sein muss. Ebenfalls müssen die
  Daten selber, welche von dem Frontend abgegriffen werden, vor dritter
  geschützt werden. Der Zugriff auf das Frontend soll über HTTPS durchgeführt
  werden können und je nach Kundenanforderung auch Self Signed Zertifikate 
  zulassen. Die Anmeldung am Frontend kann entweder über einen Lokalen
  Administrator, als auch über eine LDAP(Lightweight Directory Access Protocol)
  Anbindung verfügen, um hier z.B. Unternehmensverzeichnisse Anbinden zu können.
  \1 Eine gute und große Community und Dokumentation über das Webfrontend.
  Damit ist gemeint, das der Auftraggeber und auch die Projektmitglieder selber,
  keine große Schulung mit dem Umgang des Frontends benötigen. Dies erleichtert
  ...
  \1 Die Datensicherung muss einen wesentlichen Aspekt bei dem Frontend sein.
  Dabei muss das Frontend, bereits schon Möglichkeiten bieten, eine
  Datensicherung des Dashboards oder ähnlichem durchführen zu können.
  Das Dashboard, wo alle Graphen und Werte hinterlegt sind, können auf ein
  externes System (FileServer o.a.) abgelegt werden und können somit einfach
  dupliziert und gesichert werden.
  \1 API Zugriff für das Frontend. Damit ist gemeint, das das Frontend die
  Möglichkeit bietet, Datensätze direkt mittels einer bereitgestellten API
  Abfrage an der Datenbank abgreifen zu können und visuell bereit zu stellen.
  Dabei blablabla (hier muss nochwas hin, fällt mir gerade aber nicht ein)
  \1 Plugin Erweiterung. Das Frontend, muss eine Plugin Erweiterung
  gewährleisten. Es dient dazu, das im späteren Verlauf, wenn das Produkt
  bereits beim Kunden eingesetzt wird, auf bestimmte Kundenwünsche direkt
  eingehen zu können und diese Mittels bereits schon zur Verfügung gestellte
  Plugins oder aber durch selbst programmierte Plugins zu realisieren.
\end{outline}
\mr%
...
\begin{outline}
  ...
  ~ Webtemplate nur aus zwei auswählbar (dark/white)
