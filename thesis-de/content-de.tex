...
ermitteln~\cite{procps}. Über das Programm free oder die Datei
\texttt{/proc/meminfo} erhält man den die Menge des vorhandenen sowie zur Zeit
...
Datenträgern. Außerdem listet die Datei die geschriebe als auch gelesene
...
\begin{lstlisting}[language=bash,caption={Parsing /proc with awk},label=lst:aw]
#!/bin/bash
...
Listing~\ref{lst:aw} benötigt. Um die Daten über das Netzwerk zu verschicken
...
\begin{tabular}{lcr}
  ...
  Anzahl Prozessorkerne        & \cmark    & \cmark \\
  Datenträger Durchsatz        & \cmark    & \cmark \\
  Netzwerk Durchsatz           & \cmark    & \cmark \\
  \bottomrule
\end{tabular}
\end{center}
\tm%
