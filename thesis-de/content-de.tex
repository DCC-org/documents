...
Beispiel, welches im Anhang~\ref{figure:grafana_general} enthalten wird, wird
...
definiert werden. In dem Projekt wird für die Daten Erhaltung eine
API~\gls{API} Schnittstelle verwendet, welche im Laufe des Projektes entwickelt
und entworfen worden ist. Die API liefert hier einen \gls{JSON}-Wert zurück.
Damit Grafana diesen als Datenquelle verwenden kann, muss hier zuvor noch ein
...
sowie eine step-by-step Anleitung zur installation dieses Plugins. Die Plugins
werden hier über einen \texttt{bash} Befehl installiert. Anschließend kann
unter Datenquelle die API angesprochen werden. Sie wird nicht mittels einer
SQL-Abfrage abgerufen, sondern gibt die Werte bereits vor. Diese muss im
Anschluss unter dem Punkt \texttt{Metrics} ausgewählt und hinzugefügt werden.
Ein Beispiel hierzu ist dem Anhang~\ref{figures:grafana_metrics} beigefügt.

Nun sollten bereits einzelne Werte im Graphen visualisiert werden. Nachdem nun
die Grundeinstellung für die Graphen konfiguriert wurden, können anschließend
weitere Feineinstellungen eingestellt werden.

Im nachfolgenden Punkt \texttt{Axes} kann definiert werden, ob eine
Beschriftung für die Y- oder X-Achse benötigt wird. Zur Verdeutlichung ist dem
Anhang~\ref{figures:grafana_axes} ein Bild hierzu beigefügt. Zusätzlich kann
hier auch die entsprechende Einheit für diese Werte definiert werden, denn die
erhaltenen Werte aus der Datenbank oder der API sind meist nur \texttt{Integer}
Werte, welche keine Einheiten übergeben bekommen.
...
werden,andernfalls werden diese in einer Reihe angezeigt. Ein Beispiel hierzu
ist dem Anhang~\ref{figures:grafana_legend} beigefügt.
...
oder alles in Ordnung ist. Im Anhang~\ref{grafana_display} ist ein Bild
...
Anhang~\ref{figures:grafana_alert} beigefügt. Die Bedingung, die Konfiguriert
