...
\begin{outline}
  ...
  \1 Der wichtigste Aspekt, bei der Performance der Weboberfläche, ist 
  darauf zu achten, das Werte die visualisiert werden aus dem Zwischengespeicher
  verwendet werden, um die Auslastung auf dem Datenhaltunssystem so gering wie
  möglich zu halten. Wenn hier Beispielsweise 500 Nutzer gleichzeit auf der
  Weboberfläche Graphen visualisieren, so dürfen hier nicht die 500 Anfragen an
  das Datenhaltungssystem weitergeleitet werden, sondern müssen aus dem
  Zwieschenspeicher des Dashboards bzw. der Weboberfläche erfolgen. Wie auch die
  Datenabfragen zum Datenhaltungssystem, sollen Graphen und Werte schnell ohne
  große Zeitdifferenzen visualisiert und angezeigt werden können. Ebenfalls,
  sollte dem Aufbau des Dashboards bei einer Aktualisierung der Seite, oder aber
  auch ein Wert z.B. der CPU  Auslastung ändert sich massiv direkt angezeigt
  werden können, ohne einen Zeitlichen unterschied merken zu können.
\end{outline}
\mr%
...
OpenSource Basis auf \gls{GitHub} liegt. Während der Evaluierung von dieser
Software ist aufgefallen, dass es sich hierbei nicht direkt um eine
