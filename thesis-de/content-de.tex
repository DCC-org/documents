...
JDBC (Java Database Connectivity) ist die Schnittstelle der JVM zu Datenbanken.
Sie stellt generische Funktionen bereit um mit Datenbanken zu kommunizieren.
Für jede Datenbank, mit der kommuniziert werden soll, wird ein Treiber
benötigt. Dies ist eine Datei, in der die spezifischen Eigenschaften eines
bestimmten \gls{DBMS} beschrieben sind. Damit ist die JDBC Schnittstelle in der
Lage mit diesem DBMS zu kommunizieren. Für Logstash gibt es ein Plugin, welches
die JDBC Schnittstelle innerhalb des JRuby Bytecodes nutzbar macht. Somit
ist Logstash in der Lage in eine Datenbank zu schreiben. Postgres bietet einen
Treiber für JDBC an~\cite{postgres-jdbc}.
...
Data Sources eingeblendet. Dies soll den Administrator zusätzlich helfen eine
...
dieser auf sechs Stunden, kann aber beliebig angepasst werden.
...
Die Automatisierung erfolgt mit einem Puppet Modul. Das Puppet Modul hat
...
Installation in die Software implementiert.  Um das Puppet Modul aufrufen zu
...
Innerhalb des Moduls wird eine "Manifest.pp" Datei erstellt und anschließend
mit dem Puppet Modul aufgerufen. In dieser ist definiert, mit welcher Version
...
erleichtert dem Administrator die Konfiguration, da dieses Modul nur einmal auf
dem Betriebssystem ausgeführt werden muss um Grafana zu installieren und
...
Im Anhang befindet sich ein Beispiel für eine solche Konfigurationsdatei.
\mr%
