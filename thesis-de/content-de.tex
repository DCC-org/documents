...
und entworfen worden ist. Die API liefert hier einen \gls{JSON}-Objekt zurück.
...
kann unter Datenquelle die API angesprochen werden. Sie wird nicht mittels
einer SQL-Abfrage abgerufen, sondern über eine \gls{HTTPS} Schnittstelle. Diese
muss im Anschluss unter dem Punkt \texttt{Metrics} ausgewählt und hinzugefügt
werden. Ein Beispiel hierzu ist dem Anhang~\ref{figure:grafana_metrics}
beigefügt.
...
\texttt{Draw options}, \texttt{Series overrides} und \texttt{Thresholds}. Der
Aufbau hiervon kann dem Anhang~\ref{grafana_display} entnommen werden. Unter
...
oder alles in Ordnung ist. Im Anhang~\ref{figure:grafana_thresholds} ist ein
Bild beigefügt, welches dies einmal verdeutlicht.
