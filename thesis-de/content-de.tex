...
Bei beiden Schnittstellen der Datenerfassungssysteme handelt es sich um Binärschnittstellen. Hierbei werden Daten nicht in einer für Menschen lesbaren
...
\section{Frontend}
Die Weboberfläche besitzt zwei Schnittstellen, welche aufgeteilt sind. Zum einen die
Abfrage der Daten an dem Datenhaltungssystem und zum anderen die der Ausgabe
und Anzeige für den User beziehungsweise den Administrator. Eine Schnittstelle
auf einem Webinterface dient in erster Linie der Anzeige von Daten, die über
...
Benutzern und dem Webserver erfolgt mit \gls{HTTPS}. Dies dient zum einem der Verschlüsselung der Verbindung von Client zu Weboberfläche und zurück und zum anderen damit die Weboberfläche selber authentifiziert werden kann.
\mr%
...
Hierbei werden für einen bestimmten Zeitraum einer Timeseries die Werte genommen
...
„Tiered Trends“ sind dies 2 Wochen. Oftmals gibt es die Anforderung Trends ad hoc zu bilden oder die Intervalle regelmäßig zu verändern. Hierzu muss eine
...
Coreutils ist eine Sammlung von Standardwerkzeugen auf jedem Linux-Betriebssystem. Sie bieten einfache Möglichkeiten zur Editierung von Dateien.
Coreutils ist mit der GPL3 lizenziert~\cite{coreutils} und zählt somit als Open-Source-Software. Verwaltet wird die Programmsammlung aktuell von der Free
...
des Systemzustands. Es ist unter der GPL lizenziert und fällt damit unter Open-Source-Software. Der aktuelle \gls{Maintainer} ist Gerlof Langeveld. Die erste
...
umgewandelt werden. Dann erhält man alle Infos, die auch die interaktive Version
anzeigt. Im Standardverhalten werden alle 10 Minuten Logs geschrieben, dieses
...
Das Sammeln aller benötigter Daten wird von collectd unterstützt. Es kann lokale Daten
...
Die Firma Zabbix LLC wurde 2005 gegründet. Sie pflegt die Open-Source-Produkte zabbix-agent (im folgenden Abschnitt Agent genannt), zabbix-server und
...
unregelmäßig neue Updates. Das letzte Release ist vom November 2016. Die Open-Source-Software steht unter der MIT Lizenz~\cite{python-diamond}.
...
überführt werden. Zum Abschluss werden die Daten ausgegeben. Dies kann passieren,
indem unter anderem in einen Cache Service, eine Message Queue oder direkt in
...
Für eine bessere Beurteilung der zu evaluierenden Systeme wurden den oben
erläuterten Punkten eine Gewichtung zugeteilt. Dies dient für die Erstellung
von einer Entscheidungstabelle während der Evaluierungsphase.

\begin{center}
\begin{tabular}{lcccc}
  \toprule
  Beschreibung & Vorhandenes Wissen & Skalierbarkeit & Umwelt & Datensicherung \\
  \midrule
  Gewichtung   & 4                  & 3              & 2      & 1 \\
  \bottomrule
\end{tabular}
\captionof{table}{Gewichtung von evaluierungs Kriterien}
\end{center}
\nl%
...
\begin{outline}
  ...
  \1 Der Prozess akzeptiert eingehende Schreibanfragen über die RESTful-Schnittstelle und updated seine Shards
...
Dateninformationen auf mehrere Server verteilt und verwaltet. Im Cassandra-Umfeld wird dies über das von dem Hersteller definierte Protokoll „gossip“
...
durch dieses Verfahren die Analyseperformance dynamisch anpassen, da das HBase-System stark auf die Aggregation und niedrigen Speicherplatzbedarf optimiert
...
Programmiermodell abgeleitet. Es handelt sich dabei um ein Hadoop-YARN-basiertes System, welches ein paralleles Abarbeiten von Aufgaben ermöglicht.
...
„Hadoop Ecosystem“ zusammengefasst. Eine vollständige Liste von Hadoop-basierenden Systemen und Projekten kann auf der offiziellen Webseite des
...
bereitstellen konnte. Zum anderen die Masse an Dokumentationen und dem damit
...
Zunächst hätte aus einem der verfügbaren Softwareelementen von dem Hadoop
Ecosystem evaluiert und ausgewählt werden gemusst. Zusammengefasst wäre das
...
\subsection{Netzwerk Protokoll Treiber}
...
RabbitMQ zwar die Software in dem Bereich mit der größten Verbreitung, aber
...
werden. Während der Suche nach Alternativen fiel auf, das auch Postgres als Bus
genutzt werden kann.
...
Somit liefert nun die API Daten an Grafana und Grafana kan nicht mehr direkt
...
\section{Postgres Autopartitioning}
...
Linter und Syntax Test, werden immer genau dann benötigt, wenn der Quellcode
immer komplexer wird. Ein Linter Test, prüft von einem Quellcode die
Formatierung und macht diesen anschließend lesbarer, damit auch Leien diesen
Quellcode gut verstehen können. Der Sinn hinter einem Linter Test ist es, das
best Practice, wie auch die Darstellung auf dem Quellcode nach einer
...
Ordnung ist. Dies bedeutet das zum Beispiel geprüft wird, ob eine Klammer
...
Der Vorteil von solchen Unit Tests mit rspec sind folgende:
