...
auf die Grafana Weboberfläche, besitzt Grafana ein API (siehe auch
Abschnitt~\ref{sec:schnittstelle_datenbereitstellung}) Plugin, welches
installiert werden kann. Mit diesem API Plugin, ist es Grafana möglich, eine
API ansprechen zu können. Dies bedeutet, dass das Datenhaltungssystem API
Statements zur Verfügung stellen kann und hiermit Graphen visualisieren kann.
Grafana kann über einen \gls{Datenbanktreiber} oder eine Schnittstelle in Form
eines Treibers einfache Statements auf das Datenhaltungssystem durchführen, um
die Daten abzugreifen.  Es ist zusätzlich eine Open-Source-Software, wodurch
der Quellcode angepasst und verändert werden darf. Dies bedarf zum Beispiel bei
Anpassungen an bestimmte Kundenumgebungen. Dadurch entstehen auch keine
Support-Kosten, da die Software nicht als ein Dienst angeboten wird. Zusätzlich
zu dem Quellcode können hier Plugins zur Erweiterung der Software in der
Programmiersprache Java entwickelt und in das System eingepflegt werden. Dies
wird zum Beispiel dann benötigt, wenn weitere Graphen, Analysen oder
Datenquellen benötigt werden.  Grafana hat eine stetig wachsende Community mit
...
Dateifreigabeserver oder lokal abgespeichert werden~\cite{grafana}.
...
Profil geschrieben, dieses befindet sich unter~\ref{logstash-profile}. Es
...
Entwicklern in einem Pull Request zugeschickt (siehe~\cite{logstash-bug}).

Die Konfiguration von collectd läuft sehr ähnlich ab. Es wurde wieder ein
Puppet Profil erzeugt, dieses befindet sich unter~\ref{collectd-profile}. Dies
installiert collectd, richtet dann das Netzwerk-Plugin ein, um Daten zu
...
Entwicklern zur Verfügung gestellt (siehe~\cite{collectd-bug}).
\tm%
