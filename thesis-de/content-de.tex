...
Graphen angepasst, damit der Graph Werte visualisieren kann. Im nachfolgenden
Abschnitt~\ref{subsec:grafen-definieren}, wird hier genauer auf die Erstellung
...
werden. Zu Beginn wird hier unter dem Punkt \texttt{General} ein
aussagekräftiger Titel, sowie Beschreibung des Graphen hinterlegt. In dem
Beispiel, wird der Graph für die Prozessor Auslastung definiert. Unter dem Punkt
...
anschließend, wie im Anhang xyz dargestellt. Dies dient zur besseren
Übersichtlichkeit, wenn ein Administrator oder die Führungsebene diese Graphen
analysieren möchten. Im zweiten Schritt werden nun die einzelnen Graphen
definiert.
...
Query} hinzugefügt werden. Hier wird zunächst eine Art \texttt{SELECT}
\gls{SQL}-Abfrage hinterlegt, welche für die entsprechende Anforderung
angepasst werden kann. Innerhalb der SQL-Abfrage wird definiert, von wo die
...
definiert werden. In dem Projekt wird für die Daten Erhaltung eine API~\gls{API}
Schnittstelle verwendet, welche im laufe des Projektes entwickelt und entworfen
worden ist. Die API liefert hier einen \gls{JSON}-Wert zurück. Damit Grafana
diesen als Datenquelle verwenden kann, muss hier zuvor noch ein Plugin
installiert werden. Es stehen viele verschiedene Plugins auf der Grafana Webseite zur Verfügung. Zu jedem Plugin gibt es hier eine kurze Erläuterung, sowie eine step-by-step Anleitung zur installation dieses Plugins. Die Plugins werden hier über einen \texttt{bash} Befehl installiert. Anschließend kann unter DatenQuelle die API angesprochen
werden. Sie wird nicht mittels einer SQL-Abfrage abgerufen, sondern gibt die
Werte bereits vor. Diese muss im Anschluss unter dem Punkt \texttt{Metrics}
ausgewählt und hinzugefügt werden.

Nun sollten bereits einzelne Werte im Graphen visualisiert
werden. Nachdem nun die Grundeinstellung für die Graphen konfiguriert wurden,
können anschließend weitere Feineinstellungen eingestellt werden.

Im nachfolgenden Punkt \texttt{Axes} kann definiert werden, ob eine Beschriftung für die  Y- oder
X-Achse benötigt wird. Zusätzlich kann hier auch die entsprechende Einheit für
diese Werte definiert werden, denn die erhaltenen Werte aus der Datenbank oder der API sind
meist nur \texttt{Integer} Werte, welche keine Einheiten übergeben bekommen.

Unter dem Menüpunkt \texttt{Legend} wird die Legende für diesen Graphen
definiert. Eingestellt werden kann hier lediglich welchen Minimalen, Maximalen,
Aktuellen und Durschnitts Wert innerhalb der Analyse ausgegeben worden ist und
ob diese Werte als Tabelle ausgegeben werden sollen. Dies erleichtert die
Übersichtlichkeit in Hinsicht, wenn hier mehrere Werte ausgegeben
werden,andernfalls werden diese in einer Reihe angezeigt.

Nachdem nun die \texttt{Axes} und \texttt{Legend} angepasst worden sind, gibt
es unter dem Punkt \texttt{Display} drei Einstellungsmöglichkeiten,
\texttt{Draw options}, \texttt{Series overrides} und \texttt{Thresholds}. Unter
dem Punkt \texttt{Draw options} kann das Aussehen des Graphen angepasst werden.
So kann bei jedem Schnittpunkt oder Veränderung des Graphen ein Punkt
hinterlegt werden. Ebenso kann definiert werden, ob der Graph zur besseren
...
diesem Menüpunkt noch den Punkt \texttt{Thresholds}. \texttt{Thresholds} sind
Markierungen, innerhalb des Graphens, wenn ein kritischer Wert erreicht worden
ist. Diese dienen dazu auf einem Blick zu erkennen, ob es hier Probleme gibt,
oder alles in Ordnung ist.Im Anhang xyz ist ein Bild beigefügt, welches dies
einmal verdeutlicht.
