...
Die Datenvisualisierung soll dem Benutzer eine Übersicht über die
...
an die Datenbank über eine API Schnittstelle, (siehe
...
ermöglicht es, den Zugriff nur für die Datenvisualisierung auf die sensiblen
...
API\hyp{}Systeme von Cloudprovidern nur in bereits vorkonfigurierten Paketen mit
...
dem Benutzer oder Administrator eine leichte und einfache Konfiguration von einzelnen
...
E-Mail an den Administrator oder weitere Benutzer zu versenden, weshalb Zabbix sehr
häufig gerade in großen Infrastrukturen zum Einsatz kommt. Die Anmeldung an der
Weboberfläche kann wahlweise mit lokalen Benutzer mit Benutzername und
...
Abfrage über eine API gewünscht sein, so kann dies ebenfalls mittels Statement
direkt auf dem Datenhaltungssystem durchgeführt werden. Um die Datensicherheit
und Ausfallsicherheit gewährleisten zu können, bietet Zabbix die Möglichkeit,
...
Nutzwertanalyse der Softwarelösungen Datadog, Gridster\hyp{}D3 und Netdata entnommen
...
mehrere virtuelle CPU\hyp{}Kerne emuliert und einer Instanz zugewiesen. So kann eine
VM 2 virtuelle CPU\hyp{}Kerne mit z.B. 2,5\si{\giga\hertz} Taktrate zugewiesen
...
Dashboards hat der Administrator oder Benutzer die Möglichkeit, den
...
werden. Um weiteren Benutzern Zugriff zu dem Dashboard zu gewähren, können
unter dem Reiter Admin > Global Users weitere Benutzer angelegt werden und
...
sowohl für lokale Benutzer als auch für LDAP\hyp{}Benutzer vergeben werden. Sollte
ein Unternehmen die LDAP-Anbindung für die Authentifizierung an Grafana
...
sind, muss der Grafana\hyp{}Dienst neu gestartet werden. Dies wird mit folgendem
Befehl durchgefüht:
...
Dashboards oder direkten URL Adressen zu Webseiten hinterlegt werden. Diese
werden anschließend, wie im Anhang~\ref{figure:grafana_description} auf der
Dashboard Übersicht dargestellt. Dies dient der besseren Übersichtlichkeit,
wenn ein Administrator oder die Führungsebene diese Graphen analysieren
möchten. Im zweiten Schritt werden nun die einzelnen Graphen definiert.
\mr%
...
liefert dabei ein \gls{JSON}-Objekt. Sie wird nicht mittels einer SQL-Abfrage
abgerufen, sondern über eine \gls{HTTPS} Schnittstelle. Innerhalb der \gls{API}
werden bereits vordefinierte Werte zurückgegeben. Wenn als Datenquelle eine
Datenbank angesprochen werden soll, so kann zunächst eine Art \texttt{SELECT}
\gls{SQL}-Abfrage hinterlegt werden. Innerhalb der \gls{SQL}-Abfrage wird
definiert, von wo die erhaltenen Daten stammen, sowie welche Werte abgerufen
werden sollen. Ebenfalls kann diese \gls{SQL}-Abfrage mit anderen \gls{SQL}
Abfragen innerhalb einer Gruppe zusammengefasst werden. Ein Beispiel hierzu
...
Für die Tests und Qualitätsicherung wurden unter anderem Unit\hyp{}Tests verwendet.
Unit\hyp{}Tests sind Software\hyp{}Tests und werden immer dann verwendet, wenn jeweils
