...
installiert Logstash, dann das JDBC Plugin für die Kommunikation mit der
Datenbank. Hierfür wird noch der Postgres Treiber benötigt, dieser wird von der
Postgres Webseite heruntergeladen. Zum Schluss wird die Logstash Konfiguration
aus~\ref{lst:logstash} erzeugt und Logstash gestartet. Hierbei stellte sich
heraus, dass das Puppet Modul für Logstash nicht zu 100\% auf Archlinux
funktioniert. Dies wurde vom Projektteam behoben. Der Fix wurde den Entwicklern
in einem Pull Request zugeschickt (siehe~\cite{logstash-bug}).

Die Konfiguration von collectd läuftsehr ähnlich ab. Es wurde wieder ein Puppet
Profil erzeugt, dieses befindet sich unter~\ref{collectd-profile}. Dies
