...
über einen Datenbanktreiber, existiert noch die Möglichkeit einer \gls{API}\@.
...
kann bei der \gls{API} der potentielle Angreifer keine eigenen Datenbankbefehle
...
Datenhaltungssystem \gls{API} Statements zur Verfügung stellen kann und hiermit
Graphen visualisieren kann. Grafana kann über einen \gls{Datenbanktreiber} oder
eine Schnittstelle in Form eines \glslink{Treiber}{Treibers} einfache
...
oder lokal abgespeichert werden (vgl.~\cite{grafana}).
...
eine \gls{API}~\ref{sec:schnittstelle_datenbereitstellung} Datenabfrage oder
...
ebenfalls mittels Statement direkt auf dem Datenhaltungssystem durchgeführt
werden. Um die Datensicherheit und Ausfallsicherheit gewährleisten zu können,
bietet Zabbix die Möglichkeit, einzelne Graphen mit Statements, wie auch das
gesamte Dashboard exportieren und importieren zu können
...
pro Prozess cached und diese dann gebündelt in einem \gls{SQL}\hyp{}Statement
zur Datenbank schickt. Der Standardwert liegt bei 125 Werten und ist
...
die Last auf der Datenbank, da dies zu wesentlich mehr
\gls{SQL}\hyp{}Statements führt.  Alternativ kann mit Logstash
\glslink{Quality of Service}{QoS} gemacht werden.  Hierbei wird auf die
eingehenden Daten ein \gls{Grok} Filter gelegt. Dieser erlaubt es Daten zu
