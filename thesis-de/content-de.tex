...
Daten in Form von Graphen. In diesem Projekt werden bewusst zwei Schnittstellen
verwendet, damit die Daten innerhalb des Datenhaltungssystem geschützt sind,
wie auch die Sicherheit der eigentliche Datenvisualisierung gewährleistet
werden kann. Im nachfolgendem werden beide Schnittstellen erläutert.
...
und werden zum Beispiel für ausgefallene \glslink{SSD}{SSDs} in einem
...
\begin{outline}
  ...
  erster Linie für den Schutz der Daten. Speziell die Anmeldung am Webinterface
  schützt den unaitorisierten Zugriff durch Dritte. Der Zugriff auf das
  ...
  der Kunden anbinden zu können. Hierrüber soll ebenfalls eine Rechtevergabe
...
\begin{outline}
  ...
  visualisieren im Fallbeispiel acht Graphen mit unterschiedlichen Werten.  Nun
  sind insgesamt 64 Graphen, welche sekündlich eine Abfrage an das
...
kritischen Festplattenstands, eine E\hyp{}Mail an den Administrator oder weitere
...
der Evaluierung hat Netdata 2,35 Gesamtpunkte erhalten, da es sehr
...
zugewiesen. So kann eine VM Zwei virtuelle CPU\hyp{}Kerne mit z.B.
...
Administrator eine detaillierte eine detaillierte Übersicht in Form eines
Graphen, wie viel CPU\hyp{}Leistung von einer virtuellen Instanz verwendet wird
und auf dem System verfügbar ist. Im Folgenden finden sich die
Akzeptanzkriterien:
...
werden über einen Kommandozeilenbefehl installiert.
...
werden. Sollte ein Unternehmen die \gls{LDAP}\hyp{}Anbindung für die
Authentifizierung an Grafana wünschen, so muss in dem Verzeichnis von der
Grafana\hyp{}Installation die Datei grafana.ini angepasst werden. Alle
Einstellungen, die in der Datei vorgenommen werden, beziehen sich auf die
Servereinstellungen, welche vom Kunden individuell angepasst werden kann.
Nachdem die Änderungen abgeschlossen sind, muss der Grafana\hyp{}Dienst neu
...
nach dem Neustart von Grafana erreichbar ist. Das Problem liegt nicht in
...
vordefinierte Werte zurückgegeben. Wenn als Datenquelle eine Datenbank
angesprochen werden soll, so kann zunächst eine Art \texttt{SELECT}
...
abgerufen werden sollen. Ebenfalls kann diese \gls{SQL}\hyp{}Abfrage mit
anderen \gls{SQL} Abfragen innerhalb einer Gruppe zusammengefasst werden.  Ein
...
Administrator dienen, da dieser die Minimal\hyp{} und Maximalenwerte, sowie
Aktuelle\hyp{} und Durchnittswerte welche innerhalb der Analyse ausgegeben
worden sind, sehen kann. Ein Beispiel hierzu ist dem
...
Min\hyp{}, Max\hyp{} oder Durschchnittswert beeinhalten, ab wann diese
Bedingung erfüllt ist. Diese wird entweder mit einem Standardtext versendet
oder kann über den Konfigurationsmenüpunkt \texttt{Notifications} individuell
angepasst werden. Zudem wird definiert, über welchen Kommunikationsweg die
