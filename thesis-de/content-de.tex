...
Beispiel~\glslink{Soft-IRQ}{Soft_IRQs}, \glslink{Interrupt}{Interrupts},
...
managment der Arbeitsauslastung auf dem Server. Dazu nutzt die Software ein
...
verhindern. Dazu wird ein Peer-to-Peer Verfahren verwendet, welches die
Dateninformationen auf mehrere Server verteilt und verwaltet. Im Cassandra
Umfeld wird dies über die von dem Hersteller definierten Protokoll ``gossip''
...
Nodes ermöglicht. Wenn auf einen einzelnen Node Dateninformationen erstellt,
modifiziert oder entfernt werden, erhält ein weitere Node in der Infrastruktur
diese Information und baut gleichzeitig ein Replikation des Objektes. 

\lstinputlisting[language=bash,caption={postgres git clone}]{../listings/cassandra-status.txt}
...
``Owns'', von 50 Prozentpunkte besitzen. Dies bedeutet, dass jeder Node jeweils
die Hälfte der Dateninformationen bereitstellt, jedoch auch die zugehörige Häfte
von dem anderen Node als Backup abspeichert. Sollte nun einer der beiden Nodes
ausfallen, könnte der jeweils andere autoamtisch einen Dateneigentum von 100
Prozentpunkten erstellen und ohne Verluste weiterarbeiten.

Je nach Node Anzahl können verschiedene Replikations-Modelle in der
...
HIER: Warum wir es dennoch nicht nutzen.
\nl%
