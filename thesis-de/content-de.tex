...
einem ArchLinux Betriebssystem wird dies Paket mit folgendem Befehl
heruntergeladen und bereits vorinstalliert:
...
vorliegende Version auf dem Arch Linux Betriebssystem.
...
wird bereits ein vordefinierter Administrator User mit Password festgelegt. Die
...
werden. Anschließend sieht man hier Bereits das "HomeDashboard" von wo aus die
...
befindet. Wenn der Administrator hier auf dem HomeDashboard befindet, so wird
hier Home und die Übersicht der einzelnen Dashboards. Sollte der Administrator
in den Einstellungen unter Datasources sein, so wird hier Data Sources
eingeblendet.  Dies soll den Administrator zusätzlich helfen eine bessere
Übersichtlichkeit zu erhalten. Auf der Rechten Seite des HomeDashboards hat der
Administrator oder User die Möglichkeit, den Aktualisierungstimer der
anzuzeigenen Daten zu verändern. Standardmäßig steht dieser auf sechs Stunden,
kann aber beliebig angepasst werden.

Hier kommt das Label rein mit Bild.
...
bietet bereits von der Installation eine gute Unterstützung von verschiedenen
Datenhaltungssystemen an. Diese können nach belieben durch sogenannte Plugins
erweitert werden. Hierzu wird im weiteren Verlauf der Implementierung
eingegangen. Nachdem hier alle Werte ausgefüllt worden sind, kann die
Datenquelle über die Schaltfläche "Add" hinzugefügt werden. Grafana prüft
bereits beim speichern der Datenquelle ob diese erreichbar ist.
...
Graphen, Tabellen oder auch Singlestats. Diese können je nach Verwendungszweck
hinzugefügt werden. Eine Beschränkung wieviele Graphen maximal auf einem
Dashboard angelegt werden können gibt es nicht.

Nachdem der Graph ausgewählt worden ist, kann dieser angepasst und bearbeitet
werden. Hierzu wird der zu editieren Graph ausgewählt und anschließend auf
"edit" geklickt. Unterhalb des Graphen öffnet sich dann ein Menü, wo der
grundsätzliche Aufbau, die Metric, die Axen und diverse andere Einstellungen
für diesen Graphen definiert werden können. Hier wird zunächst die Metric des
...
werden. Dieses vorgehen wird nun für jeden Graphen erstellt, dessen Wert
visualisiert werden soll. Nachdem alle Graphen erstellt worden sind, muss das
...
werden. Um weitere User Zugriff zu dem Dashboard zu gewähren, können unter dem
Grafana Symbol > Admin > Global Users weitere User Zugriff gewährt werden.
Sollte ein unternehmen die LDAP Anbindung für die Authentifizierung an Grafana
...
Grafana. Um hier jeodch nicht bei jedem einzelnen Kunden diese Anpassung
