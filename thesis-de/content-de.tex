...
Datenhaltungssystem, wie z.B. influxDB zusammen und ist Multi Tenant fähig.
Dies bedeutet, dass sowohl ein LDAP als auch eine Benutzernamen plus Kennwort
Lösung eingesetzt werden kann. Somit können z.B. Personen aus der Führungsebene
Eingeschränkte Zugriffe aus das System erhalten, um hier Analysen oder
Marketing Statistiken durchführen zu können. Zusätzliche zu der Abfrage mit
Benutzernamen und Kennwort zur Anmeldung auf die Grafana Weboberfläche, ist 
Grafana API fähig, dies bedeutet, dass das Datenhaltungssystem API Statements
zur Verfügung stellen können und hiermit Graphen visualisieren kann. Grafana
kann ebenfalls auch über einfache Statements auf das Datenhaltungssystem die
Daten abgreifen. Es wird keine Lizenz für Grafana benötigt, wodurch der
Quellcode angepasst und verändert werden darf. Dies bedarf z.B. bei Anpassungen
an bestimmte Kundenumgebungen. Zusätzlich zu dem Quellcode, können hier
zusätzliche Plugins in der Porgrammiersprache Java entwickelt und in das System
eingepflegt werden, wenn hier weitere Graphen, Analysen oder ähnlichem benötigt
werden. Grafana hat eine stetig wachsende Community mit sämtlichen Dokumenten
zur Einrichtung oder Erweiterung des Systems. Der gesamte Quellcode ist auf
GitHub veröffentlicht, somit haben auch andere Mitglieder von GitHub, die 
Möglichkeit, das Produkt zu verbessern und neue Funktionen zu implementieren.
Dies bedarf lediglich die Zustimmung, durch die Mitglieder des Produktes auf
GitHub. Die vom Anwender erstellen Dashboards, mit allen Graphen und Statements,
können vom System, wie auch vom Anwender selber auf einen Externen
Dateifreigabe Server oder Lokal abgespeichert werden.
...
Wiki Board, wo alle relevatnen Dokumentationen enthalten sind. Graphen können
leicht mittelts Statements über eine API oder direkt an das
...
Es können einzelnen Graphen, wie auch das gesamte Dshboard auf externe Server
oder Lokal abgespeichert werden um hier ein Verlust von den einzelnen Graphen
vorbeugen zu können. Graphite ist wie auch Grafana kompatibel zu TimeSeries
Datenhaltungssystemen. Es bietet keine einfache Möglichkeit, weitere
Plugins hinzufügen zu können, wie es beispielsweise Grafana dies kann. Hierzu 
muss der Quellcode von Graphite massiv angepasst werden, was zu einer sehr 
guten Kenntnis des Systems vorraussetzt.
...
Graphen ermöglicht. Es bietet zudem eine inhaltlich gute Wissensdatenbank, wo
viele Dokumente zur Konfiguration von einzelnen Funktionen bis hin zur
Erläuterungen von jedem Button enthalten sind.
Das Zabbix-Frontend benötigt keinerlei Plugins, da hier bereits viele Funktionen
in der eigentlichen Software enthalten ist. Plugins können dennoch in der 
Programmiersprache Java entwickelt und implementiert werden. Es ist darauf
ausgelegt, bei verschiedenen Events z.B. ein kritischer Festplattenstand wurde
erreicht, eine Email an den Administrator oder weitere User zu versenden, 
weshalb Zabbix sehr häufig gerade in großen Infrastrukturen zum Einsatz kommt.
Die Anmeldung an der Weboberfläche kann wahlweise mit einem Lokalen User mit
Benutzername und Kennwort oder mit einer LDAP Anbindung durchgeführt werden.
So benötigt der Administrator, bzw. der User lediglich seine Active Directory
Benutzerdaten zur Authentifizierung. Zabbix ist zudem mit API Abfragen
...
Statement direkt auf dem Datenhaltungssystem durchgeführt werden.
Um die Datensicherheit und Ausfallsicherheit gewährleisten zu können, bietet
Zabbix die Möglichkeit, einzelne Graphen mit Statements, wie auch das gesamte
Dashboard exportieren und importieren zu können. 
...
in großen Firmen, wie z.B. EA oder Citrix erfolgreich eingesetzt. Es ist nicht
wie die anderen Software Lösungen ein OpenSource Produkt und wird in
verschiedenen Lizenzen von der Anzahl der entsprechenden Hosts gestaffelt. Die 
Basis Lizenz gibt es hier bereits kostenlos, jedoch können hier nur maximal
5 Hosts abgefragt werden und die Daten werden maximal einen Tag zurückgehalten.
Die teuerste Lizenz kostet hier 23 Doller und beinhaltet alle Host, welche
der Kunde anbinden möchte. Zusätzlich bietet diese Lizenz einen Email, Chat oder
Telefon Support. Dieser ist nicht in der Basis Lizenz vorhanden. 
Graphen und auch Dashboards können in Datadog einfach vom Administrator erstellt
werden. Um die Anmeldung an der Weboberfläche durchführen zu können, wird ein
Benutzer und Kennwort benötigt. Es ist hier nicht möglich, ein LDAP an die
Weboberfläche anbinden zu können. Innerhalb eines Graphen, können bestimmte 
Teilabschnitte mit z.B. Social Media Plattformen, wie Facebook oder Twitter
geteilt werden. Datadog bietet zudem eine Eventbasierte Mitteilungen, welche
vom Administrator festgelegt werden können. Datadog hat eine vollständige API
Unterstützung und Dashboards können vom Administrator Export werden. Eine
Möglichkeit, Graphen zu exportieren bietet Datadog hier nicht. Die Community,
wo viele Inhaltliche Dokumente enthalten sind, sind hier nur für Kunden 
bestimmt, welche eine Pro Lizenz besitzen. Ebenso gibt es hier einen IRC 
Channel, um direkt kontakt mit Support aufnehmen zu können.
...
Graphen oder anderen Visualisierungen darzustellen. Der komplette Code steht
...
Seite angezeigt, weshalb die Übersicht stark eingeschränkt ist. Graphen 
können bei Netdata nicht einfach angepasst werden, da diese im Quellcode
verändert und angepasst werden müssen. Das Netdata läuft ebenfalls nur auf Linux
Oberflächen. Ein Support von Mircrosoft Windows ist hier nichtgeben. Zusätzlich
kann hier nur jeweils eine Instanz überwacht werden. Vorteil ist hierbei, das 
keine weitere Datenbank oder ähnliches benötigt wird.
